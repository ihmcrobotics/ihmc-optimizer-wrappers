...
public class QuadProgWrapper
{
    ...
    */
   public static native double solveNative(double[] G, double[] g0, double[] CE, double[] ce0, double[] CI, double[] ci0, double[] x, int[] nIteration,
                                           byte[] errMsg);

   private void initialize(int nVariables, int nEqualityConstraints, int nIneqalityConstraints)
   {
      if ((this.nVariables != nVariables) || (this.nEqualityConstraints != nEqualityConstraints) || (this.nInequalityConstraints != nIneqalityConstraints))
      {
   ...
   public int solve(DMatrixRMaj Q, DMatrixRMaj f, DMatrixRMaj Aeq, DMatrixRMaj beq, DMatrixRMaj Ain, DMatrixRMaj bin, DMatrixRMaj x, boolean initialize)
   {
      ...
      objVal = solveNative(Q.getData(), f.getData(), Aeq.getData(), beq.getData(), Ain.getData(), bin.getData(), x.getData(), nIteration, errMsg);
      if (nIteration[0] < 0)
