...
namespace uQuadProgPP {
	...
	// linear systems
	bool cholesky_decomposition(ublas::matrix<double>& A, std::ostringstream& errorMsg);
	...
	bool solve_quadprog(ublas::matrix<double>& G, ublas::vector<double>& g0,
		const ublas::matrix<double>& CE, const ublas::vector<double>& ce0,
		const ublas::matrix<double>& CI, const ublas::vector<double>& ci0,
		ublas::vector<double>& x, int& iter, std::ostringstream& errorMsg, double& objectiveValue)
	{
		iter = 0;
		{
			...
			if (G.size2() >= mx || G.size1() >= mx ||
				...
				errorMsg << "The dimensions of one of the input matrices or ublas::vectors were "
					<< "too large." << std::endl
		...
		if ((int)G.size1() != n)
		{
			errorMsg << "The ublas::matrix G is not a square ublas::matrix (" << G.size1() << " x "
				<< G.size2() << ")";
			return false;
		}
		if (CE.size2() > 0 && (int)CE.size1() != n)
		{
			errorMsg << "The ublas::matrix CE is incompatible (incorrect number of rows "
				<< CE.size1() << " , expecting " << n << ")";
			return false;
		}
		if ((int)ce0.size() != p)
		{
			errorMsg << "The ublas::vector ce0 is incompatible (incorrect dimension "
				<< ce0.size() << ", expecting " << p << ")";
			return false;
		}
		if (CI.size2() > 0 && (int)CI.size1() != n)
		{
			errorMsg << "The ublas::matrix CI is incompatible (incorrect number of rows "
				<< CI.size1() << " , expecting " << n << ")";
			return false;
		}
		if ((int)ci0.size() != m)
		{
			errorMsg << "The ublas::vector ci0 is incompatible (incorrect dimension "
		...
		if (!cholesky_decomposition(G, errorMsg))
		{
			iter = -1;
			return false;
		}
		...
		for (i = 0; i < p; i++)
		{
			...
			if (!add_constraint(R, J, d, iq, R_norm))
			{
				...
				errorMsg  << "Constraints are linearly dependent";
				objectiveValue = f_value;
				return false;
			}
		}
			...
			for (j = i; j < n; j++)
			{
				...
				if (i == j)
				{
					if (sum <= 0.0)
					{
						// raise error
						print_matrix("A", A);
						errorMsg << "Error in cholesky decomposition, sum: " << sum;
						return false;
					}
					A(i, i) = ::std::sqrt(sum);
				}
